...
[![Tests](https://img.shields.io/badge/tests-763%20passing-brightgreen.svg)](#testing)
...
| `evaluation/test_*.py` | 59 | discovery-benchmark metric math (precision/recall/Jaccard), **multi-run stats** (mean ± 95 % CI), **arp-scan / netdiscover / masscan** baseline parsers, **privileged (ARP) baseline** command/summary, **detection-benchmark scoring** (ports/services/planted-CVE precision·recall vs a pinned testbed), **AI-copilot eval** (grounding vs. coverage scoring, novel-CVE-hallucination detection, any-of fact matching, **multi-run mean ± CI**) |

**763 tests, all green.** Static analysis is clean: **ruff** 0 findings, **ESLint**
