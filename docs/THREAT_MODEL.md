# PurpleRecon — Threat Model

A security tool must be able to account for its own posture. This document states
what PurpleRecon protects, the trust boundaries it sits across, the threats that
...
|---|---|
| The host running PurpleRecon | Must never be turned against itself (self-DoS) |
...
| T4 | **Scanning out of scope / public space** | Public/internet-routable targets are **refused by default**; opt-in via `PURPLERECON_ALLOW_PUBLIC=1`. CLI requires confirmation for public/large scopes | `vet_target`, `confirm_scope` |
| T5 | **Scan-as-a-service abuse / resource exhaustion** | Per-process **concurrency cap** (`PURPLERECON_MAX_SCANS`, default 4 → excess gets `429`); host cap per request | `backend/security.scan_slot` |
| T6 | **Unauthorized API access (if exposed)** | Optional **bearer token** (`PURPLERECON_API_TOKEN`); localhost-only bind by default; CORS limited to the dev origin + GET/POST | `backend/security.token_ok`, `app.py` CORS |
...
- **Before exposing the API beyond localhost**: set `PURPLERECON_API_TOKEN`, keep
  `PURPLERECON_ALLOW_PUBLIC` unset, and front it with TLS.

## 6. Authorization (legal)

PurpleRecon is for assets you **own or are explicitly authorized to test**.
