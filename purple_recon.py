...
try:
    ...
    sys.stderr.write(
        "\n[EnumGrid] Missing required dependency 'rich'.\n"
        ...
        if not hosts:
            if blocked:
                ...
                raise ScopeError(
                    ...
                    f"EnumGrid refuses to scan protected address space."
                )
...
def write_report(report: dict, output_dir: str) -> str:
    ...
    path = os.path.join(output_dir, f"enumgrid_{safe_target}_{stamp}.json")
...
def _timestamped_path(report: dict, output_dir: str, ext: str) -> str:
    """Build the standard ``enumgrid_<target>_<stamp>.<ext>`` output path."""
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    safe_target = _sanitize_filename(str(report.get("target", "scan")))
    return os.path.join(output_dir, f"enumgrid_{safe_target}_{stamp}.{ext}")
...
def load_baseline(path: str) -> dict:
    ...
    if not isinstance(data, dict) or "hosts" not in data:
        raise ValueError("not a EnumGrid report (missing 'hosts')")
    return data
...
def cli() -> None:
    """Console-script entry point (``enumgrid`` once pip-installed).
