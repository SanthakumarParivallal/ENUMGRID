...
_DB: list[tuple[tuple[str, ...], object, str, float, str]] = [
    ...
    (("openssh", "opensshd"), lambda v: (8, 5) <= v < (9, 8), "CVE-2024-6387", 8.1,
     "OpenSSH 8.5p1 to 9.7p1 regreSSHion signal-handler RCE"),
    # --- Web servers -------------------------------------------------------- #
    ...
    (("nginx",), lambda v: (0, 5, 6) <= v <= (1, 3, 9), "CVE-2013-2028", 7.5,
     "nginx 0.5.6 to 1.3.9 chunked-encoding stack overflow"),
    (("lighttpd",), lambda v: (1, 4, 46) <= v <= (1, 4, 66), "CVE-2022-41556", 7.5,
     "lighttpd 1.4.46 to 1.4.66 mod_wstunnel use-after-free"),
    ...
    (("webmin",), lambda v: (1, 890) <= v <= (1, 920), "CVE-2019-15107", 9.8,
     "Webmin 1.890 to 1.920 unauthenticated RCE (password_change.cgi)"),
    # --- Mail / DNS / file sharing / IRC ------------------------------------ #
    (("exim",), lambda v: (4, 87) <= v <= (4, 91), "CVE-2019-10149", 9.8,
     "Exim 4.87 to 4.91 remote command execution"),
    (("samba", "smbd"), lambda v: (3, 5, 0) <= v < (4, 6, 4), "CVE-2017-7494", 9.8,
     "Samba 3.5.0 to 4.6.x is_known_pipename() RCE (SambaCry)"),
            ...
            out.append(
                Vuln(
                    ...
                    output=(
                        ...
                        "verify, because distros may backport the fix)"
                    ),
