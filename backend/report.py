...
def _esc(value) -> str:
    ...
    ``xml.sax.saxutils``, because these are output encoders, not XML parsers; doing it
...
def _num(value) -> float | None:
    ...
    can arrive as a string, or as garbage. This keeps the renderer's "a partial
...
def build_pdf(payload: dict) -> bytes:
    ...
    # scan). Rendered as escaped text, so the model output never becomes markup.
